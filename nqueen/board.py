...
class Board():
    ...
    def create(self):
        #Tạo n x n bàn cờ dựa trên giá trị kích thước.
        ...
        self.panel = cv2.cvtColor(self.panel, cv2.COLOR_RGBA2BGRA)

    def put(self, piece: str, cell: tuple) -> bool:
        #Đặt quân cờ mong muốn vào ô mong muốn trên bàn cờ.
        row, column = cell
        self.board[row][column]['piece'] = piece
        return True

    def draw(self):
        #Vẽ và hiển thị các quân cờ và bảng trên hình ảnh
        for row_index, row in enumerate(self.board):
            for column_index, column in enumerate(row):
                if not column['piece']:
                    continue
